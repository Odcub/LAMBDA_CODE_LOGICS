...
for k, luafile in ipairs( ENTFiles ) do
    ...
    local fullpath = "lambdaplayers/lambda/" .. luafile

    -- Skip if file doesn't exist in the LUA search path or is empty
    if not file.Exists( fullpath, "LUA" ) or ( file.Size and file.Size( fullpath, "LUA" ) == 0 ) then
        print( "Skipping missing/empty file: " .. fullpath )
        continue
    end
...
-- Localization
    ...
    local adaptivethink = GetConVar( "lambdaplayers_lambda_adaptivethink" )
...
function ENT:Initialize()
    ...
    if ( SERVER ) then
        ...
        -- NEW: Personality Check Timers
        self.l_NextCuriosityCheck = CurTime() + LambdaRNG( 2, 10 )
        self.l_NextPlayfulnessCheck = CurTime() + LambdaRNG( 10, 30 )
        ...
        self:SimpleTimer( 0.2, function()
            ...
            -- NEW: Play connected voice line on spawn/join
...
-- NEW: Handle disconnect voice line when the entity is removed
function ENT:OnRemove()
    if SERVER and !self:GetIsDead() then
        self:PlaySoundFile( "disconnect" )
    end
    
    -- --- START OF FIX ---
    -- Client-side cleanup for the projected texture (flashlight)
    if CLIENT then
        local flashlight = self.l_flashlight
        if IsValid( flashlight ) then
            flashlight:Remove()
            self.l_flashlight = nil -- Set the variable to nil after removal
        end
    end
    -- --- END OF FIX ---
    
    -- Run standard cleanup hooks
...
function ENT:Think()
    ...
    if ( SERVER and !isDead ) then
        ...
        if curTime >= self.l_nextidlesound then
            if !isDisabled and !self.l_preventdefaultspeak and !self:GetIsTyping() and !self:IsSpeaking() then
                if self:GetState() == "Investigating" then
                    self:PlaySoundFile( "investigate" )
                elseif self:GetState() == "Escorting" or self:GetState() == "FollowPlayer" then
                    self:PlaySoundFile( "escorting" )
                elseif LambdaRNG( 100 ) <= self:GetVoiceChance() then
        ...
        if curTime >= self.l_nextsurroundcheck then
            ...
            -- NEW: Curiosity Check
            -- If Idle, check for interesting props or sounds to investigate
            if self:GetState() == "Idle" and curTime >= self.l_NextCuriosityCheck then
                 self.l_NextCuriosityCheck = curTime + LambdaRNG( 5, 15 )
                 local chance = ( self.GetCuriosityChance and self:GetCuriosityChance() ) or 30
                 if LambdaRNG( 100 ) <= chance then
                     local nearbyInteresting = self:FindInSphere( nil, 1000, function( ent )
                        return ( ent:GetClass():find("prop_") or ent:GetClass():find("item_") or ent:IsNPC() ) and self:CanSee( ent )
                     end )
                     if #nearbyInteresting > 0 then
                        self:SetState( "Investigating", nearbyInteresting[ LambdaRNG( #nearbyInteresting ) ] )
                     end
                 end
            end
            ...
            if nearNextbot then
                -- NEW: If confidence is low, Hide instead of just retreating blindly
                local confidence = ( self.GetConfidenceChance and self:GetConfidenceChance() ) or 30
                if confidence < 30 and self:IsInRange( nearNextbot, 800 ) then
        ...
        if !isDisabled then
            ...
            local isPanicking = ( behavState == "Retreat" or behavState == "Hiding" or behavState == "MovingToCover" )

            if LambdaIsValid( target ) and ( isPanicking or behavState == "Combat" or behavState == "MovingToCover" ) then
                ...
                if !isPanicking and endTime > 0 and curTime >= endTime then
                    ...
                    -- NEW: Combat Personality Logic
                    -- If low confidence and taking damage/outnumbered, switch to Hiding or MovingToCover
                    if !isPanicking and behavState == "Combat" and LambdaRNG(100) < 5 then
                        local confidence = ( self.GetConfidenceChance and self:GetConfidenceChance() ) or 30
                        if confidence < 40 then
                             self:SetState( "MovingToCover", target )
                        end
                    end
                    --
    ...
    if ( CLIENT ) then
        ...
        -- Add this block inside the existing ENT:OnRemove() function
        local flashlight = self.l_flashlight
        ...
        elseif IsValid( flashlight ) then
            flashlight:Remove()
            self.l_flashlight = nil -- Ensure the reference is cleared
            if !isDead and beingDrawn then self:EmitSound( "HL2Player.FlashLightOff" ) end
        end
    end

    -- Think Delay: use convar in singleplayer, otherwise use adaptive/staggered nextthink
    do
        ...
        -- Adaptive think interval for multiplayer to reduce server spikes.
        local state = ( self.l_BehaviorState or self:GetState() )
        local baseInterval = 0.18

        if state == "Combat" then
            baseInterval = 0.06
        elseif state == "Retreat" or state == "MovingToCover" or self:IsPanicking() or state == "Hiding" then
            baseInterval = 0.08
        ...
        -- Adjust by nearest player's distance: be more responsive when players are close
        local minSqr = 1e12
        ...
        if minSqr < ( 500 * 500 ) then
            baseInterval = baseInterval * 0.65
        elseif minSqr > ( 2000 * 2000 ) then
            baseInterval = baseInterval * 1.5
        end

        -- Stagger think times across entities to avoid spikes
        local stagger = ( ( self:EntIndex() % 10 ) * 0.01 )
        local nextT = curTime + baseInterval + stagger
        self:NextThink( nextT )
        return true
    end
end
...
function ENT:RunBehaviour()
    if !self.l_initialized then
        ...
        -- New: Announce our connection/spawn (using the registered 'connected' voice type)
        if !self:GetIsDead() and LambdaRNG( 100 ) < self:GetVoiceChance() then
            self:PlaySoundFile( "connected", false )
        end
        -- End New Code
