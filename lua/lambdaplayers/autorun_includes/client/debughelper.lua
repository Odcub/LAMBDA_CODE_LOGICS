...
-- Fix: Make sure this table exists on the client.
LambdaPersonalProfiles = LambdaPersonalProfiles or {}
...
hook.Add("PreDrawEffects", "lambdaplayers-debughelper", function()
    ...
    for k, lambda in ipairs(ents.FindByClass("npc_lambdaplayer")) do
        if IsValid(lambda)
        and lambda:IsBeingDrawn()
        and lambda:GetPos():DistToSqr(LocalPlayer():GetPos()) < (600 ^ 2) then

            if not lambda.l_debughelperspawndelay then
            ...
            if hp == "NAN" then
                hp = lambda:GetNWFloat("lambda_health", "NAN")
            end
            ...
            cam.Start3D2D(pos, ang, scale:GetFloat())
                ...
                -- Bools
                AddTextToQueue(queue,
                    "LAMBDA IS PROFILE: " ..
                    tostring(LambdaPersonalProfiles[lambda:GetLambdaName()] ~= nil), boolcol)

                AddTextToQueue(queue,
                    "LAMBDA IS DEAD: " .. tostring(lambda:GetIsDead()), boolcol)

                AddTextToQueue(queue,
                    "LAMBDA IS DISABLED: " ..
                    tostring(lambda:GetNW2Bool("lambda_isdisabled", false)), boolcol)

                -- Strings
                AddTextToQueue(queue,
                    "LAMBDA TEXT PROFILE: " ..
                    lambda:GetNW2String("lambda_tp", "{ UNAVAILABLE }"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA VOICE PROFILE: " ..
                    lambda:GetNW2String("lambda_vp", "{ UNAVAILABLE }"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA LAST STATE: " ..
                    lambda:GetNW2String("lambda_laststate", "{ UNAVAILABLE }"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA STATE: " ..
                    lambda:GetNW2String("lambda_state", "{ UNAVAILABLE }"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA COROUTINE STATUS: " ..
                    lambda:GetNW2String("lambda_threadstatus", "{ UNAVAILABLE }"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA CURRENT PROFILE PICTURE: " ..
                    (lambda:GetProfilePicture() or "N/A"), stringcol)

                AddTextToQueue(queue,
                    "LAMBDA CURRENT WEAPON: " ..
                    lambda:GetWeaponName(), stringcol)

                -- Entities
                AddTextToQueue(queue,
                    "LAMBDA ENEMY: " .. tostring(lambda:GetEnemy()), entitycol)

                -- Numbers
                local initTime = lambda.debuginitstart or SysTime()
                AddTextToQueue(queue,
                    "LAMBDA TIME SINCE INITIALIZE: " ..
                    string.NiceTime(math.Round(SysTime() - initTime, 2)), color_white)
                ...
                for i = 1, #queue do
                    local t = queue[i]
                    draw.DrawText(t[1], "Trebuchet24", 0, 30 - (30 * i), t[2], TEXT_ALIGN_CENTER)
                end
            ...
            -- Draw PFP
            lambda.l_debugpfpcache =
                lambda.l_debugpfpcache or Material(lambda:GetProfilePicture() or "")
