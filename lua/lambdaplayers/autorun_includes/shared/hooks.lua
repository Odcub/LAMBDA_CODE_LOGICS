...
-- Safe lambda-specific hook helpers (won't replace existing hook.Run)
if not LambdaAddHook then
    function LambdaAddHook( id, name, func )
        if not hook or not hook.Add then return end
        hook.Add( id, "lambdaplayers_addhook_" .. name .. "_" .. tostring( CurTime() ), func )
    end
end

if not LambdaRemoveHook then
    function LambdaRemoveHook( id, hookname )
        if not hook or not hook.Remove then return end
        hook.Remove( id, hookname )
    end
end