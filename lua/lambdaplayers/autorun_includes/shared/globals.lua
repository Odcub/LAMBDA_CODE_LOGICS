...
-- Lightweight registries and helpers used by extaddon modules
LambdaVoiceTypes = LambdaVoiceTypes or {}            -- table: voiceType -> true
LambdaPersonalityPresets = LambdaPersonalityPresets or {} -- name -> function( ply, lambda ) returning table
LambdaExtraStates = LambdaExtraStates or {}          -- name -> true

-- Safe wrapper for hook.Run so extaddons can use LambdaRunHook if not previously defined
LambdaRunHook = LambdaRunHook or function( name, ... )
    if hook and hook.Run then
        return hook.Run( name, ... )
    end
end

-- Register a new voice type (idempotent)
function LambdaRegisterVoiceType( voicetype )
    if not voicetype or type( voicetype ) ~= "string" then return end
    LambdaVoiceTypes[ voicetype ] = true
end

-- Register a personality preset (callable to build a personality table)
function LambdaRegisterPersonality( name, func )
    if not name or type( name ) ~= "string" or type( func ) ~= "function" then return end
    LambdaPersonalityPresets[ name ] = func
end

-- Register a simple additional state name (for bookkeeping)
function LambdaRegisterState( statename )
    if not statename or type( statename ) ~= "string" then return end
    LambdaExtraStates[ statename ] = true
end