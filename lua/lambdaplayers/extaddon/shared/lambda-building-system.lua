...
-- How many of a specific classname a lambda can have. Default is unlimited (0)
_LAMBDAPLAYERS_SpawnLimits = _LAMBDAPLAYERS_SpawnLimits or {}

-- Returns count of spawned of given class by this lambda
function EntMeta:GetSpawnedCount( classname )
    if not self.l_SpawnedEntities then return 0 end
    local cnt = 0
    for ent, cls in pairs( self.l_SpawnedEntities ) do
        if IsValid( ent ) and cls == classname then cnt = cnt + 1 end
    end
    return cnt
end

-- Test whether we can spawn this classname (checks limit)
function EntMeta:CanSpawnEntity( classname )
    local limit = _LAMBDAPLAYERS_SpawnLimits[ classname ] or 0
    if limit <= 0 then return true end
    return ( self:GetSpawnedCount( classname ) < limit )
end

-- Spawn and track an entity created by this lambda. Returns entity or nil.
function EntMeta:SpawnEntity( classname, pos, ang )
    if not classname or not self:CanSpawnEntity( classname ) then return nil end
    ...
    self.l_SpawnedEntities = self.l_SpawnedEntities or {}
    self.l_SpawnedEntities[ ent ] = classname

    LambdaRunHook( "LambdaOnSpawnedEntity", self, ent, classname )
    return ent
end

-- Cleans up spawned entities recorded by this lambda
function EntMeta:CleanSpawnedEntities()
    if not self.l_SpawnedEntities then return end
    for ent, _ in pairs( self.l_SpawnedEntities ) do
