...
-- FIX START: Safe registration to prevent nil error if loaded out of order
local function RegisterMediaWatchModule()
    if LambdaCreatePersonalityType then
        LambdaCreatePersonalityType( "MediaWatch", Mediawatch )
    end
    if LambdaRegisterVoiceType then
...
if LambdaCreatePersonalityType then
    RegisterMediaWatchModule()
else
    -- If the function doesn't exist yet, wait for Initialize
    hook.Add( "Initialize", "LambdaMediaWatch_SafeRegister", RegisterMediaWatchModule )
end
-- FIX END
...
LambdaMediaPlayers = LambdaMediaPlayers or {} -- simple registry: lambda -> { url = str, title = str, playing = bool }

local EntMeta = FindMetaTable( "Entity" ) or {}

function EntMeta:OpenMediaPlayer( url, meta )
    if not url or url == "" then return false end
    self.l_Media = self.l_Media or {}
    self.l_Media.url = url
    self.l_Media.meta = meta or {}
    self.l_Media.playing = true

    LambdaMediaPlayers[ self ] = self.l_Media
    LambdaRunHook( "LambdaOnMediaPlay", self, url, meta )
    return true
end

function EntMeta:StopMediaPlayer()
    if not self.l_Media then return end
    self.l_Media.playing = false
    LambdaMediaPlayers[ self ] = nil
    LambdaRunHook( "LambdaOnMediaStop", self )
end

function EntMeta:IsMediaPlaying()
    return self.l_Media and self.l_Media.playing or false
end