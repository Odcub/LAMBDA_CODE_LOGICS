            ...
            if random( 3 ) == 1 then
                ...
                elseif stickTogether:GetBool() then
                    for _, ent in RandomPairs( ents_GetAll() ) do
                        ...
                        -- Play a regroup voiceline when moving to teammates
                        if !self.l_preventdefaultspeak and LambdaRNG( 100 ) <= self:GetVoiceChance() and !self:IsSpeaking() then
                            self:PlaySoundFile( "regroup" )
                        end
                        if LambdaRNG( 100 ) <= self:GetTextChance() and !self:IsSpeaking() and self:CanType() then
                            self:TypeMessage( self:GetTextLine( "regroup" ) )
                        end
...
LambdaTeamSystem = LambdaTeamSystem or {
    teams = {} -- name -> { members = { ents } }
}

local function ensureTeam( name )
    LambdaTeamSystem.teams[ name ] = LambdaTeamSystem.teams[ name ] or { members = {} }
    return LambdaTeamSystem.teams[ name ]
end

function LambdaTeamCreate( name )
    if not name then return end
    return ensureTeam( tostring( name ) )
end

function LambdaTeamAddMember( name, ent )
    if not name or not IsValid( ent ) then return end
    local t = ensureTeam( name )
    t.members[ ent ] = true
    LambdaRunHook( "LambdaOnTeamJoin", name, ent )
end

function LambdaTeamRemoveMember( name, ent )
    local t = LambdaTeamSystem.teams[ name ]
    if not t then return end
    t.members[ ent ] = nil
    LambdaRunHook( "LambdaOnTeamLeave", name, ent )
end

function LambdaTeamBroadcast( name, funcname, ... )
    local t = LambdaTeamSystem.teams[ name ]
    if not t then return end
    for ent, _ in pairs( t.members ) do
        if IsValid( ent ) and ent[ funcname ] then
            ent[ funcname ]( ent, ... )
        end
    end
end

-- Helper to get team members list
function LambdaGetTeamMembers( name )
    local t = LambdaTeamSystem.teams[ name ]
    if not t then return {} end
    local out = {}
    for ent, _ in pairs( t.members ) do
        if IsValid( ent ) then table.insert( out, ent ) end
    end
    return out
end
