...
-- Start a conversation between two lambdas/entities. Returns true if started.
function EntMeta:StartConversation( other )
    if not IsValid( other ) or other == self then return false end
    -- simple mutual flag
    self.l_ConversationPartner = other
    other.l_ConversationPartner = self

    LambdaRunHook( "LambdaOnConversationStart", self, other )
    return true
end

-- End conversation
function EntMeta:EndConversation()
    local partner = self.l_ConversationPartner
    if IsValid( partner ) then partner.l_ConversationPartner = nil end
    self.l_ConversationPartner = nil
    LambdaRunHook( "LambdaOnConversationEnd", self, partner )
end

-- Send a short text exchange (uses existing Say/GetTextLine)
function EntMeta:SendConversationLine( text )
    if not text or text == "" then return end
    local partner = self.l_ConversationPartner
    if not IsValid( partner ) then return end

    -- speaker says, partner may reply via their TypeMessage or PlaySoundFile
    self:TypeMessage( text )
    LambdaRunHook( "LambdaOnConversationLine", self, partner, text )
end