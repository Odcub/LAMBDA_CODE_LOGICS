...
table.Merge( _LAMBDAPLAYERSWEAPONS, {
    badtrip = {
        ...
        OnAttack = function( self, wepent )
            ...
            self:EmitSound( "goofylambdas/badtrip_hit.mp3", nil, self:GetVoicePitch(), nil, CHAN_VOICE )
            ...
            self:SimpleWeaponTimer( waitTime, function()
                ...
                self:ApplyEffect("Bloodlust", math.huge, 400, 3)
                self:ApplyEffect("Bleeding", math.huge, 2)
                self:EmitSound("goofylambdas/badtrip.mp3", 80)
