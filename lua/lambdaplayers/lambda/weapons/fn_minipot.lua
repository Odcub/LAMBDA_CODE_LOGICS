...
table.Merge( _LAMBDAPLAYERSWEAPONS, {
    fn_minipot = {
        model = "models/fortnite/w_fbr_smallshieldpotion.mdl",
        origin = "Fortnite",
        prettyname = "Small Shield Potion",
        holdtype = "slam",
        keepdistance = 400,
        attackrange = 900,
        islethal = false,
        ...
        OnHolster = function( self, wepent )
            self:StopSound("fortnite/smallshieldpotion_drink.ogg")
            end,
        ...
        OnAttack = function( self, wepent )
            self:EmitSound( "fortnite/smallshieldpotion_drink.ogg", 90)
            self:RemoveGesture( ACT_HL2MP_GESTURE_RANGE_ATTACK_SLAM )
            self:AddGesture( ACT_HL2MP_GESTURE_RANGE_ATTACK_SLAM )
           self:SimpleWeaponTimer( 2.10, function()
                self:ApplyEffect("Energized", 0.2, 25, 1, 50)
                self:StopSound("fortnite/smallshieldpotion_drink.ogg")
                self:EmitSound("fortnite/slurpjuice_drinkcomplete.ogg", 90)
           end)
          self.l_WeaponUseCooldown = ( CurTime() + 2.10 )
