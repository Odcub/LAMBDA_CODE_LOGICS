...
local escortOptions = { autorun = true }
function ENT:Escorting( target )
    if !LambdaIsValid( target ) then
        return "Idle" -- Return to idle if the target is no longer valid
    end

    -- Play the 'escorting' voice line occasionally
    -- Set a minimum 15-second cooldown to prevent chat spam
    if LambdaRNG( 100 ) < self:GetVoiceChance() and CurTime() > ( self.l_last_escort_voiceline or 0 ) + 15 then
        self:PlaySoundFile( "escorting", 0.5 ) -- 0.5 is a low priority
        self.l_last_escort_voiceline = CurTime()
    end

    -- Move to a position near the target (e.g., 150 units away)
    local followPos = target:GetPos() + ( self:GetPos() - target:GetPos() ):GetNormalized() * 150
    self:MoveToPos( followPos, escortOptions )

    return true -- Continue Escorting state
end
