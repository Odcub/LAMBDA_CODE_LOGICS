...
--[[             -- NW Vars --
    ...
    function ENT:GetVoiceLine( voicetype )
        -- Respect master and per-type toggles (created in autorun_includes/shared/d_registervoicetypes.lua)
        local master = GetConVar( "lambdaplayers_voice_enabled" )
        if master and not master:GetBool() then return false end

        local percv = GetConVar( "lambdaplayers_voice_" .. voicetype .. "_enabled" )
        if percv and not percv:GetBool() then return false end
        ...
        if self.l_VoiceProfile and LambdaVoiceProfiles[ self.l_VoiceProfile ] then
            ...
            elseif not vpFallback:GetBool() then
                return false
            end
        end
